...
def get_embeddings(user_id):
    ...
    if user_embeds:
        ...
        for user_embed in user_embeds:
            ...
            sql_query = text("SELECT uuid FROM langchain_pg_embedding WHERE collection_id = :collection_id")
            collection_uuid = session.execute(sql_query, {"collection_id": collection_id}).scalar()
            session.close()
